...
def home(request):
    ...
    if request.user.has_perm('service.can_disable_mailings'):
        mailing_list = Mailing.objects.filter(status=2 | 3)  # Все активные или созданные рассылки
        mailing_all = Mailing.objects.all()  # Все рассылки, созданные в сервисе
    else:
        mailing_list = Mailing.objects.filter(status=2 | 3,
    ...
    }  # Создание контекста для передачи в render
